...
export const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setLoading(true);
    // Simulate API authentication/registration delay
    setTimeout(() => {
      setLoading(false);
      onLogin();
    }, 1500);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden bg-[#020617]">
      ...
      <div className="w-full max-w-md bg-white/5 backdrop-blur-2xl border border-white/10 p-10 rounded-[2rem] shadow-2xl relative z-10 animate-fade-in transition-all duration-300">
        <div className="flex flex-col items-center mb-8 space-y-3">
          ...
          <p className="text-slate-400 text-center">
            {isSignUp ? 'Join thousands of students learning smarter' : 'Sign in to unlock your full potential'}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {error && (
            <div className="p-4 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 text-sm text-center font-medium animate-in fade-in">
              {error}
            </div>
          )}
          ...
          <button 
            type="submit" 
            disabled={loading}
            className="w-full bg-gradient-to-r from-primary to-accent hover:from-accent hover:to-primary text-white py-4 rounded-2xl font-bold text-lg shadow-lg shadow-primary/25 transition-all transform hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center gap-2 mt-2"
          >
            {loading ? <Loader2 className="w-6 h-6 animate-spin" /> : <>{isSignUp ? 'Create Account' : 'Sign In'} <ArrowRight className="w-5 h-5" /></>}
          </button>
        </form>
